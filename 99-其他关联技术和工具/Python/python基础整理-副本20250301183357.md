...
60. **标准库**（python自带的模块，可以直接调用）
61. **开源模块**（第三方模块,需要先pip安装,再调用）
62. **自定义模块**（自己定义的模块）
...
print(hello.funct3()) # 调用hello模块里的funct3()函数，因为结果为函数返回值，所以还需要print打印
```
...
![1542792471040](https://minioapi.nerubian.cn/image/20250207141744824.png)



![1542792646331](https://minioapi.nerubian.cn/image/20250207141751022.png)



![1542793157401](https://minioapi.nerubian.cn/image/20250207141754982.png)

![1542793229015](https://minioapi.nerubian.cn/image/20250207141804479.png)

![1542793382389](https://minioapi.nerubian.cn/image/20250207141828890.png)

![1542793474906](https://minioapi.nerubian.cn/image/20250207141808756.png)
...
def auto_ssh(filepath):
    f = open(filepath, mode="r")
    data = f.readlines()
    f.close()

    for line in data:
        read_ip = line.strip().split(":")[0]
        read_port = line.strip().split(":")[1]
        if read_port == 22:
            ssh(read_ip)
        else:
            ssh(read_ip, read_port)
```
...
![1556453501201](https://minioapi.nerubian.cn/image/20250207141852772.png)



![1541320609631](https://minioapi.nerubian.cn/image/20250207141856413.png)



![1541320800203](https://minioapi.nerubian.cn/image/20250207141900823.png)
...
![1556454296969](https://minioapi.nerubian.cn/image/20250207141904734.png)



![1556454437271](https://minioapi.nerubian.cn/image/20250208104454739.png)



![1556454521151](https://minioapi.nerubian.cn/image/20250207141909346.png)
...
![1556457026497](https://minioapi.nerubian.cn/image/20250207141916920.png)
...
![1556457345947](https://minioapi.nerubian.cn/image/20250207141925151.png)

![1556457548011](https://minioapi.nerubian.cn/image/20250207141929067.png)

![1556457697508](https://minioapi.nerubian.cn/image/20250207141933036.png)
...
```python
import paramiko					# 模块里也可以导入其它模块

# ssh_exec_cmd函数用于远程执行命令
def ssh_exec_cmd(hostname, command, port=22):
    ssh = paramiko.SSHClient()
    private_key = paramiko.RSAKey.from_private_key_file("/root/.ssh/id_rsa")
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy)
...
def auto_ssh(filepath):
    ...
    f = open(filepath, mode="r")
    data = f.readlines()				# 读取文件所有内容，做成列表，每行内容为列表中的元素
    f.close()
    ...
    for line in data:
        ...
        if read_port == 22:
            ssh_exec_cmd(read_ip,cmd)	# 如果port为22,则不用传port参数，因为默认值参数为22
        else:
            ssh_exec_cmd(read_ip, cmd, read_port)  # port不为22，则需要传port
```
...
* 如果执行命令没有结果输出，两个都可以
* 如果是为了得到命令的结果输出，并想对结果赋值进行后续操作的话，就使用os.popen(cmd).read()
* 如果直接得到命令结果就可以了，那么直接使用os.system(cmd)就OK
...
## 标准库之time,datetime,calendar模块(拓展)
...
![1541313363756](https://minioapi.nerubian.cn/image/20250207142013628.png)
...
Python代码混淆是为了防止代码被轻易逆向工程或篡改，常用的方法可以分为以下几类。
...
### **注意事项**
- **性能影响**：混淆可能增加代码体积和执行时间。
- **不可逆性**：部分混淆操作不可逆，需保留原始代码。
- **法律风险**：某些国家和地区对代码混淆有法律限制。
- **局限性**：没有绝对安全的混淆，关键算法建议用C/C++实现并通过接口调用。
